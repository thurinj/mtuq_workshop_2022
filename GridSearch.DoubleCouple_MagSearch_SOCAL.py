...
if __name__=='__main__':
    #
    # Carries out grid search for a double couple solution for the catalog depth and a magnitude search of the 2020-04-04 SoCal event
    # Uses local database of Green's functions calculated via the FK method
    #
    # USAGE
    #   mpirun -n <NPROC> python GridSearch.DoubleCouple_MagSearch_SOCAL.py
    #
    ...
    grid = DoubleCoupleGridRegular(
        npts_per_axis=30,
        magnitudes=[4.7,4.8,4.9,5.0,5.1])
    ...
    if comm.rank==0:
        ...
        plot_data_greens2(event_id+'DC_MagSearch_waveforms.png',
            data_bw, data_sw, greens_bw, greens_sw, process_bw, process_sw,
            misfit_bw, misfit_sw, stations, origin, best_source, lune_dict)


        plot_beachball(event_id+'DC_MagSearch_beachball.png',
            best_source, stations, origin)


        plot_misfit_dc(event_id+'DC_MagSearch_misfit.png', results)
        ...
        save_json(event_id+'DC_MagSearch_solution.json', merged_dict)


        # save misfit surface
        results.save(event_id+'DC_MagSearch_misfit.nc')
